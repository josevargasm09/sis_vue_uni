  ...
  <v-dialog v-model="dialog" max-width="1200px" persistent>
    <v-card>
      <v-card-title class="headline grey lighten-2">
        {{ localReport.id ? 'Editar Reporte de Ventas' : 'Agregar Reporte de Ventas' }}
        <v-spacer></v-spacer>
      ...
      <v-card-text>
        <v-form ref="form">
          <v-row>
            ...
            <v-col cols="12" md="6">
              <v-text-field
                v-model="localReport.totalSales"
...
export default {
  ...
  methods: {
    ...
    saveReport() {
      if (!this.localReport.date || !this.localReport.totalSales) {
      ...
      const saveMethod = this.localReport.id ? SalesReportService.updateReport : SalesReportService.createReport;
