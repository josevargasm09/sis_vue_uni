  ...
  <v-dialog v-model="dialog" max-width="1200px" persistent>
    <v-card>
      <v-card-title class="headline grey lighten-2">
        {{ localReport.id ? 'Editar Reporte de Clientes' : 'Agregar Reporte de Clientes' }}
        <v-spacer></v-spacer>
      ...
      <v-card-text>
        <v-form ref="form">
          <v-row>
            <v-col cols="12" md="6">
              <v-text-field
                v-model="localReport.reportDate"
                label="Fecha"
                outlined
                dense
                required
            ...
            <v-col cols="12" md="6">
              <v-text-field
                v-model="localReport.totalClients"
...
export default {
  ...
  methods: {
    ...
    saveReport() {
      if (!this.localReport.reportDate || !this.localReport.totalClients) {
      ...
      const saveMethod = this.localReport.id ? ClientReportService.updateReport : ClientReportService.createReport;
