<template>
  <v-container>
    ...
    <PurchaseReportFormModal ref="reportFormModal" @updateList="fetchReports" />
...
import PurchaseReportFormModal from './PurchaseReportFormModal.vue';
import NotificationSnackbar from '../../../components/NotificationSnackbar.vue';

export default {
  components: {
    PurchaseReportFormModal,
    NotificationSnackbar
  },
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'Compras Totales', value: 'totalPurchases' },
  ...
  methods: {
    fetchReports() {
      PurchaseReportService.getAllReports().then(response => {
    ...
    confirmDeleteReport() {
      PurchaseReportService.deleteReport(this.reportToDelete.id).then(() => {
        this.fetchReports();
    ...
    searchReport() {
      if (this.reportQuery.length > 0) {
        PurchaseReportService.searchReports(this.reportQuery).then(response => {
          this.reports = response.data;
        }).catch(error => {
